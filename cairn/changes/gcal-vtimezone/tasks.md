...
- [x] Cargo.toml: jiff with `tzdb-bundle-always` behind the `gcal` feature, defaults off so the host's zoneinfo is never read.
- [x] src/gcal/timezone.rs: an IANA name to the VTIMEZONE it denotes around an anchor, built from the transitions bracketing it, a yearly rule where the following ones agree, a settled zone as one observance, and the name test the stash decision reads.
